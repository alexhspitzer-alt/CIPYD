...
function render(node) {
  ...
  for (let [key,val] of Object.entries(props)) {
    ...
    if (key === 'fill' || key === 'priority' || key === 'key' || key.startsWith('on') || val == null || val === false) continue;
...
const hydration = `<script>(async()=>{for(const img of document.querySelectorAll('img[src$=".base64"]')){const b64=(await fetch(img.src).then(r=>r.text())).replace(/\\s/g,'');img.src=URL.createObjectURL(await fetch('data:image/jpeg;base64,'+b64).then(r=>r.blob()))}for(const [name,file] of [['Geist','geist-latin.woff2'],['Geist Mono','geist-mono-latin.woff2']]){const b64=(await fetch('assets/source/fonts/'+file+'.base64').then(r=>r.text())).replace(/\\s/g,'');const font=new FontFace(name,'url(data:font/woff2;base64,'+b64+')');await font.load();document.fonts.add(font)}document.documentElement.style.setProperty('--font-geist-sans','Geist');document.documentElement.style.setProperty('--font-geist-mono','Geist Mono')})()</script>`;
